  ...
  (
    ...
    return (
      <aside
        ref={ref}
        className={cn(
            "hidden md:flex flex-col h-full duration-300 ease-in-out transition-[width]",
...
>(({ className, ...props }, ref) => (
  <div
    ref={ref}
    data-sidebar="menu"
    className={cn("flex w-full min-w-0 flex-row md:flex-col gap-1 p-2 md:p-0", className)}
    {...props}
  />
))
  ...
  (
    ...
    const buttonContent = (
      <div className={cn("flex items-center gap-2", isMobile && "flex-col text-xs")}>
          {children}
          <span className={cn("flex-1 transition-opacity duration-200", (state === 'collapsed' && !isMobile) && 'sr-only' )}>{props.title}</span>
      </div>
  )
    ...
    const commonProps = {
        ...
        className: cn(
          ...
          className,
          isMobile && "flex-col !h-auto !p-2 data-[active=true]:bg-transparent data-[active=true]:text-primary data-[active=true]:font-bold"
        ),
        ...props
    };
